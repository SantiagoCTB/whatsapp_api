...
def _ensure_ia_config_table(cursor):
    cursor.execute(
        ...
        CREATE TABLE IF NOT EXISTS ia_config (
            ...
            state_alert_timeout_minutes INT NULL,
            state_alert_target_number VARCHAR(30) NULL,
            state_alert_template_name VARCHAR(100) NULL,
            state_alert_template_language VARCHAR(20) NULL,
    ...
    cursor.execute("SHOW COLUMNS FROM ia_config LIKE 'state_alert_timeout_minutes';")
    if cursor.fetchone() is None:
        cursor.execute(
            "ALTER TABLE ia_config ADD COLUMN state_alert_timeout_minutes INT NULL AFTER unattended_alert_template_language;"
        )

    cursor.execute("SHOW COLUMNS FROM ia_config LIKE 'state_alert_target_number';")
    if cursor.fetchone() is None:
        cursor.execute(
            "ALTER TABLE ia_config ADD COLUMN state_alert_target_number VARCHAR(30) NULL AFTER state_alert_timeout_minutes;"
        )

    cursor.execute("SHOW COLUMNS FROM ia_config LIKE 'state_alert_template_name';")
    if cursor.fetchone() is None:
        cursor.execute(
            "ALTER TABLE ia_config ADD COLUMN state_alert_template_name VARCHAR(100) NULL AFTER state_alert_target_number;"
        )

    cursor.execute("SHOW COLUMNS FROM ia_config LIKE 'state_alert_template_language';")
    if cursor.fetchone() is None:
        cursor.execute(
            "ALTER TABLE ia_config ADD COLUMN state_alert_template_language VARCHAR(20) NULL AFTER state_alert_template_name;"
        )
...
def configuracion_ia():
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            state_alert_target_number = (
                request.form.get('state_alert_target_number') or ''
            ).strip() or None
            state_alert_template_name = (
                request.form.get('state_alert_template_name') or ''
            ).strip() or None
            state_alert_template_language = (
                request.form.get('state_alert_template_language') or ''
            ).strip() or 'es_CO'
            ...
            state_alert_timeout_minutes = request.form.get('state_alert_timeout_minutes')
            try:
                state_alert_timeout_minutes = int(state_alert_timeout_minutes)
            except (TypeError, ValueError):
                state_alert_timeout_minutes = None
            if state_alert_timeout_minutes is not None and state_alert_timeout_minutes < 0:
                state_alert_timeout_minutes = 0
                    ...
                    c.execute(
                        ...
                        UPDATE ia_config
                           SET model_name = %s,
                               ...
                               state_alert_timeout_minutes = %s,
                               state_alert_target_number = %s,
                               state_alert_template_name = %s,
                               state_alert_template_language = %s
                         WHERE id = %s
                        """,
                        (
                            ia_model,
                    ...
                    c.execute(
                        ...
                        INSERT INTO ia_config
                            (model_name, model_token, system_prompt, enabled, business_description,
                             ...
                             state_alert_timeout_minutes, state_alert_target_number,
                             state_alert_template_name, state_alert_template_language)
