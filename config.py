import os
import json
import re


def _load_chat_state_definitions():
    """Return sanitized chat state configuration from environment."""

    raw_definitions = os.getenv('CHAT_STATE_DEFINITIONS')
    try:
        definitions = json.loads(raw_definitions) if raw_definitions else None
    except json.JSONDecodeError:
        definitions = None

    if not definitions:
        definitions = [
            {
                "key": "esperando_respuesta",
                "label": "Esperando respuesta",
                "color": "#f0ad4e",
                "text_color": "#1f1f1f",
            },
            {
                "key": "asesor",
                "label": "Asesor",
                "color": "#28a745",
                "text_color": "#ffffff",
            },
            {
                "key": "en_flujo",
                "label": "En flujo",
                "color": "#0d6efd",
                "text_color": "#ffffff",
            },
            {
                "key": "inactivo",
                "label": "Inactivo",
                "color": "#6c757d",
                "text_color": "#ffffff",
            },
            {
                "key": "error_flujo",
                "label": "Error de flujo",
                "color": "#dc3545",
                "text_color": "#ffffff",
            },
        ]

    sanitized = []
    seen_keys = set()
    for entry in definitions:
        if not isinstance(entry, dict):
            continue

        key = entry.get("key")
        if not isinstance(key, str):
            continue

        normalized_key = re.sub(r"[^a-z0-9_-]+", "_", key.strip().lower()).strip("_")
        if len(normalized_key) > 20:
            normalized_key = normalized_key[:20]
        if not normalized_key or normalized_key in seen_keys:
            continue

        label = entry.get("label")
        if not isinstance(label, str) or not label.strip():
            label = normalized_key.replace("_", " ").title()

        color = entry.get("color")
        text_color = entry.get("text_color")
        color = color.strip() if isinstance(color, str) else ""
        text_color = text_color.strip() if isinstance(text_color, str) else ""

        sanitized.append(
            {
                "key": normalized_key,
                "label": label,
                "color": color or "#666666",
                "text_color": text_color or "#ffffff",
            }
        )
        seen_keys.add(normalized_key)

    required = [
        {
            "key": "asesor",
            "label": "Asesor",
            "color": "#28a745",
            "text_color": "#ffffff",
        },
        {
            "key": "en_flujo",
            "label": "En flujo",
            "color": "#0d6efd",
            "text_color": "#ffffff",
        },
    ]

    for entry in required:
        key = entry["key"]
        if key in seen_keys:
            continue
        sanitized.append(entry)
        seen_keys.add(key)

    return sanitized

class Config:
    # Las sesiones deben expirar al cerrar el navegador (no persistentes).
    SESSION_PERMANENT = False
    SECRET_KEY = os.getenv('SECRET_KEY')
    META_TOKEN = os.getenv('META_TOKEN')
    MESSENGER_TOKEN = os.getenv('MESSENGER_TOKEN')
    INSTAGRAM_TOKEN = os.getenv('INSTAGRAM_TOKEN')
    PHONE_NUMBER_ID = os.getenv('PHONE_NUMBER_ID')
    PAGE_ID = os.getenv('PAGE_ID')
    PAGE_ACCESS_TOKEN = os.getenv('PAGE_ACCESS_TOKEN')
    PLATFORM = os.getenv('PLATFORM')
    MESSENGER_PAGE_ID = os.getenv('MESSENGER_PAGE_ID')
    MESSENGER_PAGE_ACCESS_TOKEN = os.getenv('MESSENGER_PAGE_ACCESS_TOKEN')
    MESSENGER_MESSAGING_TYPE = os.getenv('MESSENGER_MESSAGING_TYPE', 'RESPONSE')
    MESSENGER_MESSAGE_TAG = os.getenv('MESSENGER_MESSAGE_TAG', '')
    MESSENGER_MEDIA_TIMEOUT_SECONDS = int(
        os.getenv('MESSENGER_MEDIA_TIMEOUT_SECONDS', 180)
    )
    MESSENGER_ATTACHMENT_TIMEOUT_SECONDS = int(
        os.getenv('MESSENGER_ATTACHMENT_TIMEOUT_SECONDS', 180)
    )
    MESSENGER_AUTOMATION_DISCLOSURE_MESSAGE = os.getenv(
        'MESSENGER_AUTOMATION_DISCLOSURE_MESSAGE',
        '',
    )
    MESSENGER_DISCLOSURE_RESET_SECONDS = int(
        os.getenv('MESSENGER_DISCLOSURE_RESET_SECONDS', 86400)
    )
    VERIFY_TOKEN = os.getenv('VERIFY_TOKEN',"my_secret_token")
    SESSION_TIMEOUT = int(os.getenv('SESSION_TIMEOUT_SECONDS', 1800))
    LOGIN_MAX_ATTEMPTS = int(os.getenv('LOGIN_MAX_ATTEMPTS', 5))
    LOGIN_WINDOW_SECONDS = int(os.getenv('LOGIN_WINDOW_SECONDS', 300))
    LOGIN_LOCKOUT_SECONDS = int(os.getenv('LOGIN_LOCKOUT_SECONDS', 900))
    INACTIVE_ASSIGNMENT_SECONDS = int(os.getenv('INACTIVE_ASSIGNMENT_SECONDS', 900))
    SESSION_TIMEOUT_MESSAGE = os.getenv(
        'SESSION_TIMEOUT_MESSAGE',
        'Tu sesión ha terminado por inactividad. Hemos reiniciado la conversación.',
    )
    IA_API_TOKEN = os.getenv('IA_API_TOKEN')
    IA_MODEL = os.getenv('IA_MODEL', 'o4-mini')
    IA_CATALOG_USE_OPENAI = os.getenv('IA_CATALOG_USE_OPENAI', 'true').strip().lower() in {
        '1',
        'true',
        'yes',
        'on',
    }
    MAX_VIDEO_MB = int(os.getenv('MAX_VIDEO_MB', 500))
    MAX_VIDEO_BYTES = MAX_VIDEO_MB * 1024 * 1024
    MAX_CONTENT_LENGTH = int(os.getenv('MAX_CONTENT_LENGTH', MAX_VIDEO_BYTES))
    WHATSAPP_VIDEO_MAX_MB = int(os.getenv('WHATSAPP_VIDEO_MAX_MB', 16))
    WHATSAPP_VIDEO_MAX_BYTES = WHATSAPP_VIDEO_MAX_MB * 1024 * 1024
    IA_CATALOG_MAX_FILE_MB = int(os.getenv('IA_CATALOG_MAX_FILE_MB', 50))
    IA_CATALOG_REQUEST_DELAY_SECONDS = float(os.getenv('IA_CATALOG_REQUEST_DELAY_SECONDS', 1.0))
    LANGUAGETOOL_URL = os.getenv('LANGUAGETOOL_URL', 'https://api.languagetool.org/v2/check')
    LANGUAGETOOL_LANGUAGE = os.getenv('LANGUAGETOOL_LANGUAGE', 'es')
    IA_SYSTEM_MESSAGE = os.getenv(
        'IA_SYSTEM_MESSAGE',
        (
            'Eres un asesor comercial humano, cálido y conversacional. Respondes en español (Colombia) ' \
            'con tono profesional y cercano. Hablas como si conocieras el portafolio de la empresa. ' \
            'Resume y reformula, evitando bloques largos literales. No reveles la procedencia de la ' \
            'información ni nombres internos. Estructura la respuesta con: un saludo breve; una frase ' \
            'de qué hace la empresa (máximo 20 palabras); 1 a 3 productos relevantes con nombre o ' \
            'modelo, beneficio principal y precio si aplica, usando bullets simples sin tecnicismos; ' \
            'y un cierre con una pregunta para avanzar (ciudad, medidas, disponibilidad, envío o forma ' \
            'de pago). Si faltan datos, pregunta solo lo mínimo necesario. El objetivo es sonar natural ' \
            'y llevar a una siguiente acción como cotizar, confirmar ciudad o agendar asesoría.'
        ),
    )
    PUBLIC_BASE_URL = os.getenv('PUBLIC_BASE_URL', '')
    WHATSAPP_EMBEDDED_SIGNUP_CONFIG_ID = os.getenv('WHATSAPP_EMBEDDED_SIGNUP_CONFIG_ID', '')
    WHATSAPP_OAUTH_REDIRECT_URI = os.getenv('WHATSAPP_OAUTH_REDIRECT_URI', '')
    WHATSAPP_EMBEDDED_SIGNUP_REDIRECT_URI = os.getenv('WHATSAPP_EMBEDDED_SIGNUP_REDIRECT_URI', '')
    EMBEDDED_SIGNUP_REDIRECT_URI = (
        WHATSAPP_EMBEDDED_SIGNUP_REDIRECT_URI
        or WHATSAPP_OAUTH_REDIRECT_URI
        or os.getenv(
        'EMBEDDED_SIGNUP_REDIRECT_URI',
        'https://app.whapco.site/configuracion/signup',
        )
    )
    PREFERRED_URL_SCHEME = os.getenv('PREFERRED_URL_SCHEME', 'https')
    IA_HISTORY_LIMIT = int(os.getenv('IA_HISTORY_LIMIT', 30))
    IA_CHAT_ACTIVE_HOURS = os.getenv('IA_CHAT_ACTIVE_HOURS', '')
    IA_CHAT_ACTIVE_TZ = os.getenv('IA_CHAT_ACTIVE_TZ', 'America/Bogota')
    INITIAL_STEP = os.getenv('INITIAL_STEP', 'menu_principal')
    MAX_TRANSCRIPTION_DURATION_MS = int(os.getenv('MAX_TRANSCRIPTION_DURATION_MS', 60000))
    TRANSCRIPTION_MAX_AVG_TIME_SEC = float(os.getenv('TRANSCRIPTION_MAX_AVG_TIME_SEC', 10))
    VOSK_MODEL_PATH = os.getenv('VOSK_MODEL_PATH')

    DB_HOST     = os.getenv('DB_HOST')
    DB_PORT     = int(os.getenv('DB_PORT', 3306))
    DB_USER     = os.getenv('DB_USER')
    DB_PASSWORD = os.getenv('DB_PASSWORD')
    DB_ROOT_PASSWORD = os.getenv('DB_ROOT_PASSWORD')
    DB_NAME     = os.getenv('DB_NAME')
    TENANT_HEADER = os.getenv('TENANT_HEADER', 'X-Tenant-ID')
    DEFAULT_TENANT = os.getenv('DEFAULT_TENANT')
    DEFAULT_TENANT_NAME = os.getenv('DEFAULT_TENANT_NAME')
    FACEBOOK_APP_ID = os.getenv('FACEBOOK_APP_ID')
    SECRET_PASSWORD_APP = os.getenv('SECRET_PASSWORD_APP')
    FACEBOOK_APP_SECRET = os.getenv('FACEBOOK_APP_SECRET') or SECRET_PASSWORD_APP
    INSTAGRAM_APP_ID = os.getenv('INSTAGRAM_APP_ID')
    INSTAGRAM_APP_SECRET = os.getenv('INSTAGRAM_APP_SECRET')
    INSTAGRAM_OAUTH_TOKEN_URL = os.getenv('INSTAGRAM_OAUTH_TOKEN_URL', '')
    INSTAGRAM_OAUTH_REDIRECT_URI = os.getenv(
        'INSTAGRAM_OAUTH_REDIRECT_URI',
        'https://app.whapco.site/configuracion/signup',
    )
    SIGNUP_FACEBOOK = os.getenv('SIGNUP_FACEBOOK')
    MESSENGER_EMBEDDED = os.getenv('MESSENGER_EMBEDDED')
    SIGNUP_INSTAGRAM = os.getenv('SIGNUP_INSTAGRAM')
    SIGNUP_INSTRAGRAM = SIGNUP_INSTAGRAM or os.getenv(
        'SIGNUP_INSTRAGRAM',
        'https://www.instagram.com/oauth/authorize?force_reauth=true&client_id=2307196559764999&redirect_uri=https://app.whapco.site/configuracion/signup&response_type=code&scope=instagram_business_basic%2Cinstagram_business_manage_messages%2Cinstagram_business_manage_comments%2Cinstagram_business_content_publish%2Cinstagram_business_manage_insights',
    )
    FACEBOOK_GRAPH_API_VERSION = os.getenv('FACEBOOK_GRAPH_API_VERSION', 'v24.0')
    DEFAULT_ADMIN_PASSWORD_HASH = os.getenv(
        'DEFAULT_ADMIN_PASSWORD_HASH',
        'scrypt:32768:8:1$JAUhBgIzT6IIoM5Y$6c5c9870fb039e600a045345fbe67029001173247f3143ef19b94cddd919996a7a82742083aeeb6927591fa2a0d0eb6bb3c4e3501a1964d53f39157d31f81bd4',
    )

    BASEDIR    = os.path.dirname(os.path.abspath(__file__))
    # La app siempre guarda los medios en ``static/uploads`` para que sea
    # sencillo montarlo como volumen persistente en Docker. No permitimos
    # sobrescribir esta ruta vía variable de entorno para evitar que los
    # archivos desaparezcan al recrear el contenedor.
    _DEFAULT_MEDIA_ROOT = os.path.join(BASEDIR, "static", "uploads")
    MEDIA_ROOT = os.path.abspath(_DEFAULT_MEDIA_ROOT)
    CHAT_STATE_DEFINITIONS = _load_chat_state_definitions()
    ENABLE_TYPING_INDICATOR = os.getenv('ENABLE_TYPING_INDICATOR', 'false').strip().lower() in {
        '1',
        'true',
        'yes',
        'on',
    }
    OCR_LANG = 'spa'
    OCR_PSM = os.getenv('OCR_PSM', '6')
    OCR_OEM = os.getenv('OCR_OEM', '3')
    OCR_MAX_DIM = int(os.getenv('OCR_MAX_DIM', 3000))
    OCR_MIN_HEIGHT = int(os.getenv('OCR_MIN_HEIGHT', 1200))
